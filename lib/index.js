...
module.exports = {
  ...
  getArticle:async function(req) {
    ...
    let arr=[]
    let db=await Database.open('./data/database.db')
    await db.each('SELECT * FROM article WHERE projectId LIKE "'+req.projectId+'" ORDER BY time DESC', (err, row)=> {
  ...
  postArticle:async function(req) {
    let db=await Database.open('./data/database.db')
    await db.run("INSERT INTO article (title,content,projectId,type,img,author,tagId,time) VALUES ('"+req.title+"','"+req.content+"','"+req.projectId+"','"+req.type+"','"+req.img+"','"+req.author+"','"+req.tagId+"','"+moment(new Date()).format('YYYY-MM-DD HH:mm:ss')+"')", (err, row)=> {
      //console.log(row)
    });
    return {success:true}
  },
  putArticle:async function(req) {
    let db=await Database.open('./data/database.db')
    await db.run("UPDATE article SET title= '"+req.title+"', content= '"+req.content+"', type='"+req.type+"', projectId='"+req.projectId+"',img='"+req.img+"',author='"+req.author+"',tagId='"+req.tagId+"' WHERE id = '"+req.id+"'", (err, row)=> {
      //console.log(row)
    });
    return {success:true}
  },
  ...
  getTagByProjectId:async function(req) {
    let arr=[]
    let db=await Database.open('./data/database.db')
    await db.each('SELECT * FROM tag WHERE projectId="'+req.projectId+'"', (err, row)=> {
      arr.push(row)
    });
    return arr
  },
